...
const COLUMNS = [
  ...
  'check_name', 'check_until', 'uid', 'updated_at', 'extra_flags', 'business_days', 'business_name',
];
...
export function importXlsx(arrayBuffer) {
  ...
  for (const r of rows) {
    ...
    const stop = {
      ...
      extraFlags: strOrNull(r.extra_flags) ? String(r.extra_flags).split(';;').filter(Boolean) : [],
      businessDays: strOrNull(r.business_days),
...
export function exportXlsx(stops, official) {
  ...
  for (const s of stops) {
    aoa.push([
      ...
      officialIndex[s.id] ?? '', s.checkName || '', s.checkUntil || '', s.uid || '', s.updatedAt ?? 0, (s.extraFlags || []).join(';;'), s.businessDays || '', s.businessName || '',
    ]);
  }
